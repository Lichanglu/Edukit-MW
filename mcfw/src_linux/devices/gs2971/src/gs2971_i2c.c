...
//	Int32 status = 0;
	...
	regAddr[0] = RegAddr;
	OSA_CPLD_i2cRead16(&gDevice_gs2971CommonObj.i2cHandle,GS2971_IIC_SLAVE_ADDR, regAddr, regValue, 2);
...
UInt16 Device_gs2971_Write16_CPLD(Device_Gs2971Obj * pObj, UInt8 RegAddr, UInt16 RegVal)
{
	...
	status = OSA_CPLD_i2cWrite16 (&gDevice_gs2971CommonObj.i2cHandle, GS2971_IIC_SLAVE_ADDR, regAddr, regValue, 2 );
...
static Int32 Device_gs2971Config(Device_Gs2971Obj *pObj, int inMode, int tmds)
{
	...
	if((gs2971_gLineNum[inMode].width != 0)&&(gs2971_gLineNum[inMode].hight != 0)
		&&(gs2971_SavEavVbi[inMode].sav != 0)&&(gs2971_SavEavVbi[inMode].e_vbi != 0))
		{
			printf("Device_gs2971Config() inMode = %d\n",inMode);
			Device_gs2971_Write16_CPLD(pObj, 16*2, gs2971_gLineNum[inMode].width);
			Device_gs2971_Write16_CPLD(pObj, 17*2, gs2971_gLineNum[inMode].hight);
			Device_gs2971_Write16_CPLD(pObj, 18*2, gs2971_SavEavVbi[inMode].sav);
			Device_gs2971_Write16_CPLD(pObj, 19*2, gs2971_SavEavVbi[inMode].e_vbi);
	}else{
		printf("SDI Std = %s \n",gs2971_gLineNum[inMode].name);
		printf("Device_gs2971Config() Errror!\n");
	}
...
//	unsigned short data;
	int val ;
	Device_gs2971Reset(pObj);
	val = Device_gs2971_Read16_CPLD(pObj,  0x18);
		printf("GS2971 VAL =%x \n",val);
	val = val|0x01;

	Device_gs2971_Write16_CPLD(pObj,  0x18 ,val) ;


	cpld_version = Device_gs2971_Read16_CPLD(pObj, 15*2);
//new hardware	
//	hsfq  = Device_gs2971_Read16_CPLD(pObj, 0*2);
//	linenum  = Device_gs2971_Read16_CPLD(pObj, 1*2);
//old hardware
	hsfq  = Device_gs2971_Read16_CPLD(pObj, 8*2);
	linenum  = Device_gs2971_Read16_CPLD(pObj, 9*2);
	printf("GS2971 cpld_version =%x \n",cpld_version);
	printf("GS2971 hsfq =%x \n",hsfq);
	printf("GS2971 linenum =%x \n",linenum);
	...
	gGS2971_Hpv= linenum;
	gGS2971_Lps = hsfq;
	if((inMode != -1)&&(gGS2971_Mode != inMode)){
		gGS2971_Mode = inMode;
		Device_gs2971Config(pObj, gGS2971_Mode, gGS2971_TMDS);
#if 0
		data = Device_gs2971_Read16_CPLD(pObj, 14*2);
		data &= 3;
		if((inMode == DEVICE_STD_1080I_60)||(inMode == DEVICE_STD_1080I_50)){
			data &= ~1;
			Device_gs2971_Write16_CPLD(pObj, 14*2,data);
		}else{
			data |= 1;
			Device_gs2971_Write16_CPLD(pObj,14*2,data);
		}
...
Int32 Device_gs2971SetDirection ( Device_Gs2971Obj * pObj,
                             unsigned int *direction )
{
	...
	if(inMode == -1)return 0;

	sav = (val&0xFFFF0000)>>16;
	e_vbi = val&0xFFFF;
	
	gs2971_SavEavVbi[inMode].sav = 	sav;
	gs2971_SavEavVbi[inMode].e_vbi = e_vbi;
//	printf("sav=%d,e_vbi=%d\n",sav,e_vbi);
	Device_gs2971_Write16_CPLD(pObj, 18*2, gs2971_SavEavVbi[inMode].sav);
	Device_gs2971_Write16_CPLD(pObj, 19*2, gs2971_SavEavVbi[inMode].e_vbi);
...
Int32 Device_gs2971GetDirection ( Device_Gs2971Obj * pObj,
                             unsigned int *direction )
{
	Int32 status = 0;
	Int32 inMode =0;
	...
	sav = gs2971_SavEavVbi[inMode].sav;
	e_vbi = gs2971_SavEavVbi[inMode].e_vbi;
...
{
	Int32           retVal = 0;
	//retVal = Device_gs2971_Read16_CPLD(pObj, 12*2);
	//retVal =retVal |0x12;
	//Device_gs2971_Write16_CPLD(pObj, 12*2,retVal );
	if(gs_cnt1)
	return 0;
	gs_cnt1=1;
