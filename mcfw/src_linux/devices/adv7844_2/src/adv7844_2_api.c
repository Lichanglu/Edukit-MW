...
{
    Device_Adv7844_2Obj *pObj = ( Device_Adv7844_2Obj * ) handle;
    Int32 status;

    if ( pObj == NULL )
        return -1;
   printf("Device_adv7844_2########################## \n");
    switch ( cmd )
    {
        case DEVICE_CMD_START:
            status = Device_adv7844_2Start ( pObj );
            break;

        case DEVICE_CMD_STOP:
            status = Device_adv7844_2Stop ( pObj );
            break;

        case IOCTL_DEVICE_VIDEO_DECODER_GET_CHIP_ID:
            status = Device_adv7844_2GetChipId ( pObj, cmdArgs, cmdStatusArgs );
            break;

        case IOCTL_DEVICE_VIDEO_DECODER_RESET:
            status = Device_adv7844_2Reset ( pObj );
            break;

        case IOCTL_DEVICE_VIDEO_DECODER_SET_VIDEO_MODE:
            status = Device_adv7844_2SetVideoMode ( pObj, cmdArgs );
            break;

        case IOCTL_DEVICE_VIDEO_DECODER_GET_VIDEO_STATUS:
            status = Device_adv7844_2GetVideoStatus ( pObj, cmdArgs, cmdStatusArgs );
            break;

        case IOCTL_DEVICE_VIDEO_DECODER_SET_VIDEO_COLOR:
            status = Device_adv7844_2SetVideoColor ( pObj, cmdArgs );
            break;

        case IOCTL_DEVICE_ADV7844_2_SET_VIDEO_NF:
            status = Device_adv7844_2SetVideoNf ( pObj, cmdArgs );
            break;

        case IOCTL_DEVICE_VIDEO_DECODER_REG_WRITE:
            status = Device_adv7844_2RegWrite ( pObj, cmdArgs );
            break;

        case IOCTL_DEVICE_VIDEO_DECODER_REG_READ:
            status = Device_adv7844_2RegRead ( pObj, cmdArgs );
            break;
			
        case IOCTL_DEVICE_VIDEO_DECODER_SET_DIRECTION:
            status = Device_adv7844_2SetDirection(pObj, cmdArgs);
            break;
			
        case IOCTL_DEVICE_VIDEO_DECODER_GET_DIRECTION:
            status = Device_adv7844_2GetDirection(pObj, cmdArgs);
            break;
			
         case IOCTL_DEVICE_VIDEO_DECODER_SET_SOURCE_CHAN:
            status = Device_adv7844_2SetSourceChan(pObj, cmdArgs);
            break;
			
        case IOCTL_DEVICE_VIDEO_DECODER_GET_SOURCE_CHAN:
            status = Device_adv7844_2GetSourceChan(pObj, cmdArgs);
            break;
			
       case IOCTL_DEVICE_VIDEO_DECODER_INVERT_CBCR:
            status = Device_adv7844_2InvertCbCr(pObj);
            break;
			
	case IOCTL_DEVICE_VIDEO_DECODER_CSC:			
            status = Device_adv7844_2_CSC(pObj,cmdArgs);
            break;		
			
         case IOCTL_DEVICE_VIDEO_DECODER_GET_HDCP:
            status = Device_adv7844_2GetHDCP(pObj, cmdArgs);
            break;
			
        case IOCTL_DEVICE_VIDEO_DECODER_EXTERN_INFORM:
            status = Device_adv7844_2GetExternInformation(pObj, cmdArgs);
            break;
...
/*
  ...
  This API does not configure the ADV7844_2 is any way.
  ...
  drvId - driver ID, must be DEVICE_VID_DEC_ADV7844_2_DRV
...
*/
Device_Adv7844_2Handle Device_adv7844_2Create ( UInt8 drvId,
                                            UInt8 instId,
                                            Ptr createArgs,
                                            Ptr createStatusArgs)
{
    Int32 status = 0;
    Device_Adv7844_2Obj *pObj;
    ...
    if ( vidDecCreateArgs->deviceI2cInstId >= DEVICE_I2C_INST_ID_MAX
         || vidDecCreateArgs->numDevicesAtPort > DEVICE_ADV7844_2_DEV_MAX )
    {
        vidDecCreateStatus->retVal = -1;
        return NULL;
    }

    pObj = (Device_Adv7844_2Obj *)malloc(sizeof(Device_Adv7844_2Obj));
    ...
    {
        memset(pObj, 0, sizeof(Device_Adv7844_2Obj));

        gDevice_adv7844_2CommonObj.adv7844_2handle[instId] = pObj;
        ...
        status = OSA_i2cOpen(&(gDevice_adv7844_2CommonObj.i2cHandle), 2);
     ...
     */
    Device_adv7844_2_SetFPGAMode(0);
...
Int32 Device_adv7844_2Delete ( Device_Adv7844_2Handle handle, Ptr deleteArgs )
{
    ...
    OSA_i2cClose(&(gDevice_adv7844_2CommonObj.i2cHandle));
...
Int32 Device_adv7844_2_SetFPGAMode(int fpgaMode)
{
    UInt8 regAddr[1]={0};
    UInt8 regValue[2]={0};
    unsigned short data;
    ...
    regAddr[0] = 14*2;
    OSA_CPLD_i2cRead16(&gDevice_adv7844_2CommonObj.i2cHandle,0, regAddr, regValue, 2);
    regValue[0] |= (1<<1);
    OSA_CPLD_i2cWrite16(&gDevice_adv7844_2CommonObj.i2cHandle,0, regAddr, regValue, 2);
	
    OSA_CPLD_i2cRead16(&gDevice_adv7844_2CommonObj.i2cHandle,0, regAddr, regValue, 2);
    data = regValue[1]<<8|regValue[0];
    printf("Device_adv7844_2Init() FPGA %d = 0x%x\n", regAddr[0], data);

    return 0;
}
/*
  System init for ADV7844_2 driver
...
Int32 Device_adv7844_2Init (  )
{
     ...
     */
    memset ( &gDevice_adv7844_2CommonObj, 0, sizeof ( gDevice_adv7844_2CommonObj ) );
