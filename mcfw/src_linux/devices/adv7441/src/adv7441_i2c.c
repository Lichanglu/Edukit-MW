...
static Device_Source_CH gAD7441_Source_Ch = DEVICE_SOURCE_CP;//DEVICE_SOURCE_ALL;
...
static Vps_Adv7441_InMode ArgMode_7441[DEVICE_STD_REACH_LAST] = {
	...
	{"1080P30",0x4b,0x61,0x5e,0x00,0xe8,0x98,0x10,0x03,0x51,0x46,0x50},	// 16-1920x1080x30p  
...
//	Int32 status = 0;
	...
	regAddr[0] = RegAddr;
	OSA_CPLD_i2cRead16(&gDevice_adv7441CommonObj.i2cHandle,CPLD_IIC_SLAVE_ADDR, regAddr, regValue, 2);
...
UInt16 Device_adv7441_Write16_CPLD(Device_Adv7441Obj * pObj, UInt8 RegAddr, UInt16 RegVal)
{
	...
	status = OSA_CPLD_i2cWrite16 (&gDevice_adv7441CommonObj.i2cHandle, CPLD_IIC_SLAVE_ADDR, regAddr, regValue, 2 );
...
static int Device_adv7441_SetPhase(Device_Adv7441Obj *pObj, int inMode)
{
	...
	if((inMode == DEVICE_STD_1080P_60) ||(inMode == DEVICE_STD_1080P_50) )
	{
		printf("---------1080P50/60----------------------\n");
//		Device_adv7442_Write8_Usermap(pObj, 0x6a, 0x00);
		Device_adv7441_Write8_Usermap(pObj, 0x6A, 0xff);
	//	Device_adv7441_Write8_Usermap(pObj, 0x37, 0x01);
...
//		if(tmds){
		if ( (DEVICE_STD_1080I_60 ==  inMode || DEVICE_STD_1080I_50 ==  inMode)){
			Device_adv7441_Write8_Usermap(pObj, 0x8C,0x01);
			Device_adv7441_Write8_Usermap(pObj, 0x8D,0x01);
			Device_adv7441_Write8_Usermap(pObj, 0x86,0x0B);
		}else{
...
#ifdef HAVE_CVBS //add by zm	
	if(inMode != gAD7441_Mode){
		Fdata = Device_adv7441_Read16_CPLD(pObj, 14*2);
		Fdata &= 3;
		data = Device_adv7441_Read8_Usermap(pObj, 0x6B);
		if((inMode == DEVICE_STD_PAL)||(inMode == DEVICE_STD_NTSC)){
			Fdata &= ~1;
			Device_adv7441_Write16_CPLD(pObj, 14*2,Fdata);
			data &= ~(1<<6);
			Device_adv7441_Write8_Usermap(pObj, 0x6B, data);
			data = Device_adv7441_Read8_Usermap(pObj, 0x7C);
			data |= (1<<5);
			Device_adv7441_Write8_Usermap(pObj, 0x7C, data);
		}else{
			Fdata |= 1;
			Device_adv7441_Write16_CPLD(pObj,14*2,Fdata);
			data |= (1<<6);
