    ...
    switch ( cmd )
    {
        ...
        case IOCTL_DEVICE_VIDEO_DECODER_EXTERN_INFORM:
         ...
         case IOCTL_DEVICE_VIDEO_DECODER_SET_DIRECTION:
            status = Device_gs2972SetDirection(pObj, cmdArgs);
            break;
			
        case IOCTL_DEVICE_VIDEO_DECODER_GET_DIRECTION:
            status = Device_gs2972GetDirection(pObj, cmdArgs);
            break;
...
Int32 Device_gs2972_SetFPGAMode(int fpgaMode)
{
    UInt8 regAddr[1]={0};
    UInt8 regValue[2]={0};
    unsigned short data;
    ...
    regValue[0] |= (1<<3);
    OSA_CPLD_i2cWrite16(&gDevice_gs2972CommonObj.i2cHandle,0, regAddr, regValue, 2);
	
    OSA_CPLD_i2cRead16(&gDevice_gs2972CommonObj.i2cHandle,0, regAddr, regValue, 2);
    data = regValue[1]<<8|regValue[0];
    printf("Device_gs2972Init() FPGA %d = 0x%x\n", regAddr[0], data);
