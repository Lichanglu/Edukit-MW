...
static Vps_Gs2972_sav_eav_vbi  gs2972_SavEavVbi[DEVICE_STD_REACH_LAST] = {
	...
	{"1080I50",	714,0x00,0x00,21},		// 12-1920x1080x50i  
	{"1080P60",	273,0x00,0x00,44},		// 13-1920x1080x60p      
	{"1080P50",	713,0x00,0x00,44},		// 14-1920x1080x50p      
	{"1080P25",	714,0x00,0x00,44},		// 15-1920x1080x25p              
	{"1080P30",	274,0x00,0x00,44},		// 16-1920x1080x30p  
	#endif
...
static Int32 Device_gs2972Config(Device_Gs2972Obj *pObj, int inMode, int tmds)
{
	...
	if((gs2972_gLineNum[inMode].width != 0)&&(gs2972_gLineNum[inMode].hight != 0)
		&&(gs2972_SavEavVbi[inMode].sav != 0)&&(gs2972_SavEavVbi[inMode].e_vbi != 0))
		{
			printf("Device_gs2972Config() inMode = %d==%d\n",inMode,gs2972_SavEavVbi[inMode].e_vbi);
			Device_gs2972_Write16_CPLD(pObj, 20*2, gs2972_gLineNum[inMode].width);
			Device_gs2972_Write16_CPLD(pObj, 21*2, gs2972_gLineNum[inMode].hight);
			Device_gs2972_Write16_CPLD(pObj, 22*2, gs2972_SavEavVbi[inMode].sav);
			Device_gs2972_Write16_CPLD(pObj, 23*2, gs2972_SavEavVbi[inMode].e_vbi);
	}else{
		printf("SDI Std = %s \n",gs2972_gLineNum[inMode].name);
		printf("Device_gs2972Config() Errror!\n");
	}
...
//old hardware
	...
	if((inMode != -1)&&(gGS2972_Mode != inMode)){
		gGS2972_Mode = inMode;
		Device_gs2972Config(pObj, gGS2972_Mode, gGS2972_TMDS);
#if 0
		data = Device_gs2972_Read16_CPLD(pObj, 14*2);
		data &= 3;
		if((inMode == DEVICE_STD_1080I_60)||(inMode == DEVICE_STD_1080I_50)){
			data &= ~2;
			Device_gs2972_Write16_CPLD(pObj, 14*2,data);
		}else{
			data |= 2;
			Device_gs2972_Write16_CPLD(pObj,14*2,data);
		}
...
Int32 Device_gs2972SetDirection ( Device_Gs2972Obj * pObj,
                             unsigned int *direction )
{
	...
	gs2972_SavEavVbi[inMode].sav = 	sav;
	gs2972_SavEavVbi[inMode].e_vbi = e_vbi;
...
Int32 Device_gs2972GetDirection ( Device_Gs2972Obj * pObj,
                             unsigned int *direction )
{
	Int32 status = 0;
	Int32 inMode = 0;
	...
	sav = gs2972_SavEavVbi[inMode].sav;
	e_vbi = gs2972_SavEavVbi[inMode].e_vbi;
                                     ...
                                     VCAP_VIDEO_SOURCE_CH_STATUS_S     * pStatus )
{
	...
	if(gGS2972_Mode != -1){
		...
		if(gGS2972_Mode < DEVICE_STD_CIF)	
			pStatus->isInterlaced = TRUE;
		if(gGS2972_Mode >DEVICE_STD_720P_50 &&  gGS2972_Mode <DEVICE_STD_1080P_60)	
			pStatus->isInterlaced = TRUE;
	}else{
